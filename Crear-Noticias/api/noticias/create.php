...
try {
    ...
    // Insertar noticia
    $stmt = $conn->prepare("
        INSERT INTO noticias (titulo, contenido, media_items, created_date, fecha_expiracion, activa, autor_id) 
        VALUES (?, ?, ?, ?, ?, 1, ?)
    ");
    
    $stmt->execute([$titulo, $contenido, $media_items, $created_date, $fecha_expiracion, $usuario_id]);
    
    $noticia_id = $conn->lastInsertId();
    
    // Obtener noticia creada para devolverla
    $stmt = $conn->prepare("

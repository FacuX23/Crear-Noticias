...
try {
    ...
    if ((int)$session['usuario_id'] === $id) {
        http_response_code(400);
        echo json_encode(['error' => 'No se puede eliminar el usuario actual']);
        exit;
    }
    ...
    if (strtolower($target['usuario'] ?? '') === 'director') {
        http_response_code(400);
        echo json_encode(['error' => 'No se puede eliminar el usuario director']);
        exit;
    }
